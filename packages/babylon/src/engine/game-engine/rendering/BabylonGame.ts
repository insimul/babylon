...
import { isElectronAI } from "@insimul/babylon/conversation";
...
import { WorldStateManager, type GameStateSource, type GameStateTarget } from "@insimul/babylon/data/WorldStateManager";
import { SaveIndicator } from "@shared/game-engine/rendering/SaveIndicator";
import { DataSource, ApiDataSource, createDataSource, type GenerationJobSummary } from "@insimul/babylon/data/DataSource";
import { ResourceProfiler } from "@insimul/babylon/data/diagnostics/ResourceProfiler";
...
export class BabylonGame {
  ...
  private async tryShowGameIntro(): Promise<void> {
    ...
    try {
      ...
      const { resolveNarrativeVariables } = await import('@shared/narrative/narrative-generator');
  ...
  private getClueMenuData(): MenuClueData | null {
    ...
    try {
      const { MAIN_QUEST_CHAPTERS } = require('@shared/quest/main-quest-chapters');
    ...
    for (let i = 0; i < scaledSettlements.length; i++) {
      ...
      try {
        ...
        if (lotsHaveStreetTopology && !lotsHavePositions) {
          try {
            const { resolveStreetLotPosition } = await import('@shared/layout-resolver');
        ...
        if (lotsHaveGridTopology && !lotsHavePositions) {
          try {
            const { resolveGridLotPosition, resolveGridParkPosition, GRID_SPACING } = await import('@shared/layout-resolver');
            const { GRID_SIZE } = await import('@shared/street-pattern-selection');
            ...
            if (!settlement.streets?.some((s: any) => s.waypoints?.length > 0)) {
              const { resolveGridStreet } = await import('@shared/layout-resolver');
