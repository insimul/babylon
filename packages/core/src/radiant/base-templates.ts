/**
 * Base radiant template pack (US-RQ5).
 *
 * The starter pack of genre-neutral radiant quest templates shipped with
 * `@insimul/core`. The canonical, portable source is the sibling data file
 * `packages/core/data/radiant/base-templates.pl` (native engines read the `.pl`
 * directly); this module mirrors it as a string constant so the browser/Babylon
 * runtime can consult it without filesystem access — the same convention as
 * `HELPER_PREDICATES_PROLOG` (helper-predicates.ts).
 *
 * A drift guard (`base-templates.test.ts`) reads the `.pl` and asserts it matches
 * this constant, so the two copies can never silently diverge.
 *
 * Loader path: a world export carrying a radiant template pack passes it to
 * `GamePrologEngine.initialize({ radiantTemplates })`, which consults it into the
 * live KB at game start (exactly like the authored `narrative_*` story templates
 * and the base rule packs). {@link BASE_RADIANT_TEMPLATES} is the default pack a
 * world without its own authored templates uses. The `RadiantQuestDirector`
 * (US-RQ3) then generates quests against the consulted templates on each tick.
 */

/**
 * The base radiant template pack as consultable Prolog source. Mirrors
 * `packages/core/data/radiant/base-templates.pl` verbatim.
 */
export const BASE_RADIANT_TEMPLATES = `% Insimul base radiant template pack (US-RQ5)
%
...
/**
 * The template ids shipped in {@link BASE_RADIANT_TEMPLATES}, in declaration
 * order. Handy for tests and for a UI that wants to enumerate the base pack.
 */
export const BASE_RADIANT_TEMPLATE_IDS = [
  'rt_fetch',
  'rt_delivery',
  'rt_bounty',
  'rt_escort',
