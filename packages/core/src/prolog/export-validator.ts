...
import { createPrologEngine } from './prolog-engine';
 ...
 *
 * Checks:
 *   1. Syntax — can the engine parse the program?
...
export async function validateExportKnowledgeBase(prologContent: string): Promise<ValidationResult> {
  ...
  // A validation pass builds a throwaway KB, so it must RELEASE it — wasm has
  // no finalizers (see PrologEngine.destroy).
  const engine = await createPrologEngine();
  try {
    // 1. Syntax check — can the engine parse it?
