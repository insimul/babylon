...
import { chatInputFocused } from "@shared/game-engine/rendering/BabylonChatPanel";
...
export class CharacterController {
    ...
    /**
     * Movement plane constraint for 2D gameplay modes (side-scroll, fighting)
     * 'free' = normal 3D movement
     * 'xy' = constrain to XY plane (side-scroller - left/right + up/down, no forward/back)
     * 'xz' = constrain to XZ plane (top-down - left/right + forward/back, no up/down except gravity)
     */
    ...
    private _doMove(dt: number): ActionData {
        ...
        // move the avatar
        if (moving) {
            // Apply movement plane constraint for 2D gameplay modes
            if (this._movementPlane === 'xy') {
                // Side-scroller: lock Z-axis (no forward/back movement)
                this._moveVector.z = 0;
            } else if (this._movementPlane === 'xz') {
                // Top-down 2D: Y is handled by gravity, this is mainly for completeness
                // Y movement (jumping/falling) is still allowed
            }
    ...
    private _onKeyDown(e: KeyboardEvent) {
        ...
        if (assessmentModalOpen || compositionModalOpen || chatInputFocused) return;
    ...
    private _addkeylistener() {
        let canvas: HTMLCanvasElement | null = this._scene.getEngine().getRenderingCanvas();
        if (!canvas) return;
